...
if which docker >/dev/null 2>&1; then
	...
	docker service create --detach=false -p 80:80 -p 443:443 -p 8383:8383 --restart-delay 30s --restart-condition on-failure --name traefik --mode replicated --constraint 'node.role == manager' --network mesos --network proxy traefik --web.address ":8383" --web.statistics --marathon.endpoint http://marathon:8080
fi
