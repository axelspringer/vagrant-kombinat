...
if which docker >/dev/null 2>&1; then
    ...
    docker network create --driver overlay --opt encrypted --attachable mesos
	docker network create --driver overlay --opt encrypted --attachable proxy
	docker service create --restart-delay 30s --restart-condition on-failure --name watchtower --mode global --mount type=bind,src=/var/run/docker.sock,dst=/var/run/docker.sock centurylink/watchtower --cleanup
