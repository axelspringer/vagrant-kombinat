...
if which docker >/dev/null 2>&1; then
    ...
    docker service create --container-label mesos=master -d -p mode=host,target=5050,published=5050 -e MESOS_PORT=5050 -e MESOS_ZK=zk://exhibitor:2181/mesos -e MESOS_QUORUM=1 -e MESOS_REGISTRY=in_memory --restart-delay 30s --constraint 'node.role == manager' --network mesos --name mesos-master --mode global --restart-condition any pixelmilk/mesos-master:1.3.0-2.0.3 --advertise_ip
	docker service create -d -e MESOS_PORT=5051 -e MESOS_MASTER=zk://exhibitor:2181/mesos -e MESOS_ADVERTISE_IP=${MANAGER_IP} -e MESOS_SWITCH_USER=0 -e MESOS_CONTAINERIZERS=docker,mesos -e MESOS_WORK_DIR=/var/tmp/mesos -e MESOS_SYSTEMD_ENABLE_SUPPORT=false --mount type=bind,src=/sys/fs/cgroup,dst=/sys/fs/cgroup --mount type=bind,src=/var/run/docker.sock,dst=/var/run/docker.sock --constraint 'node.role == worker' --network mesos --name mesos-agent --mode global --restart-condition any pixelmilk/mesos-agent:1.3.0-2.0.3
